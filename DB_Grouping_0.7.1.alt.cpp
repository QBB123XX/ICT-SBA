...
void add(){
	...
	add_seed();
	cout << "Is the student's data correct? (Y/N)";
	getline(cin, inp);
	if (inp == "Y" || inp == "y") {
...
void group_randomize(int l, int r) {
    int swap_idx;
    srand(time(NULL));
...
void make_bracket(int l, int r) {
    ...
    if (distinct) {
        ...
        //split the range into 2 parts and call the function again
        vector <student> a, b;
        for (int i = l; i <= r; i += 2) {
...
void create_tournament_bracket() {
    ...
    do {
        ...
        //randomise order of students in each school
        int l = 0, r = 0;
