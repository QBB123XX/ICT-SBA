...
void save_bracket(int round_std_cnt) { //saves current results to results file
    ...
    for (match m: bracket) { // save each match in results file
        student* std1 = m.s1;
        student* std2 = m.s2;
    ...
    if (std_cnt > 1) {
        ...
        for (int i = 0; i < round_std_cnt/2; i++) { // save each match in rounds file
            match m = bracket[i];
            student* std1 = m.s1;
            student* std2 = m.s2;
...
int setup() { //prepares std_list & bracket for file input
    ...
    while (results_file.peek() != EOF) {
        ...
        results_file.get();
        //deal with matches with bye
        if (temp_match.s1 == &bye || temp_match.s2 == &bye) {
            if (temp_match.s1 == &bye) {
                temp_match.winner = 2;
            } else {
                temp_match.winner = 1;
            }
